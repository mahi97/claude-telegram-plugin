...
// PID lock: only one instance polls; others serve tools only (outgoing replies still work).
const holdsPollLock = acquirePollLock()
if (!holdsPollLock) {
  process.stderr.write('telegram channel: another instance is polling — this instance serves tools only (no getUpdates)\n')
} else {
